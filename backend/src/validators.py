...
def join_name_parts(parts: List[str], sep: str = " > ") -> str:
    parts = [clean(p) for p in parts if clean(p)]
    # normalize to a single canonical separator style for outputs
    return f"{clean(sep).strip() if sep else ' > '}".join([""] + parts)[1:] if False else " > ".join(parts)
...
def extract_cybba_l1_l2_leaf(name: str, provider: str, sep: str) -> Tuple[str, str, str, str]:
    ...
    return provider_tok, l1, l2, leaf


# ✅ NEW: allow taxonomy model / generator to pass predicted taxonomy columns,
# while still enforcing the business naming convention.
def _sync_taxonomy_columns_from_name(
    row: pd.Series,
    *,
    provider: str,
    sep: str,
    prefer_name_over_model: bool = True,
) -> pd.Series:
    """
    Ensures taxonomy columns (if present) are consistent with the final normalized name.

    - If "Proposed New Segment Name" is valid/normalized, we can derive L1/L2 from it.
    - If taxonomy prediction columns exist, we either:
        A) overwrite them from the name (default, safest), OR
        B) keep model values unless missing (set prefer_name_over_model=False)

    This does NOT drop any working columns; it only adds/aligns if present.
    """
    name = clean(row.get("Proposed New Segment Name", ""))
    p, l1, l2, leaf = extract_cybba_l1_l2_leaf(name, provider=provider, sep=sep)
    if not p:
        return row  # nothing to sync

    # Canonical, cleaned values (must match enforcement behavior)
    l1_fixed = title_case_node(l1)
    l2_fixed = title_case_node(l2)

    # Common column candidates we may want to sync
    l1_cols = ["Predicted L1", "Taxonomy L1", "L1"]
    l2_cols = ["Predicted L2", "Taxonomy L2", "L2"]
    l1c_cols = ["Predicted L1 Confidence", "L1 Confidence", "Taxonomy L1 Confidence"]
    l2s_cols = ["Predicted L2 Similarity", "L2 Similarity", "Taxonomy L2 Similarity"]

    # If columns exist, either force-sync (default) or only fill blanks
    def _write_if_present(cols: List[str], value: str):
        nonlocal row
        for c in cols:
            if c in row.index:
                if prefer_name_over_model:
                    row[c] = value
                else:
                    if not clean(row.get(c, "")):
                        row[c] = value

    _write_if_present(l1_cols, l1_fixed)
    _write_if_present(l2_cols, l2_fixed)

    # Confidence/similarity: do NOT invent values; only keep existing.
    # But if someone wrote them as non-numeric strings, we preserve them as-is.
    # (No changes here.)
...
def enforce_clean_segment_names(
    ...
    ✅ Also preserves/aligns any taxonomy columns the generator may have attached.
    """
    ...
    for _, row in proposals_df.iterrows():
        ...
        # ✅ NEW: keep taxonomy columns consistent with the normalized name
        # Default behavior: prefer name-derived L1/L2 if those columns exist.
        try:
            new_row = _sync_taxonomy_columns_from_name(
                new_row, provider=provider, sep=sep, prefer_name_over_model=False
            )
        except Exception as e:
            # Never fail naming; just log and continue.
            logger.warning("Taxonomy column sync failed (non-fatal): %s", e)
...
def resolve_name_collisions(
    ...
    # ✅ NEW: after collision resolution, if taxonomy columns exist, keep them aligned to name
    # (Collision suffix is on leaf; L1/L2 should remain stable.)
    try:
        provider = clean(cfg["provider"]["cybba_name"])
        sep = clean(cfg.get("taxonomy", {}).get("separator", " > "))
        out = out.apply(
            lambda r: _sync_taxonomy_columns_from_name(r, provider=provider, sep=sep, prefer_name_over_model=False),
            axis=1,
        )
    except Exception as e:
        logger.warning("Taxonomy column sync after collision resolution failed (non-fatal): %s", e)
...
def validate_and_prepare(
    ...
    *,
    ...
    ✅ Preserves any extra columns produced upstream (taxonomy predictions, similarity scores, etc.)
    """
    logger = logger or logging.getLogger(__name__)
