...
def predict_prices(
    ...
    *,
    ...
    quantile_widen: float = 0.10,
    hard_clip: Optional[Dict[str, Tuple[float, float]]] = None,
    # ✅ NEW: controls how "hard" quantile guardrails feel
    squash_temp_mult: float = 3.0,
) -> pd.DataFrame:
    """
    1) kNN pricing
    ...
    knn_df = _knn_predict_prices(model, audience, tax_l1, query_text, k=int(k), min_sim=float(min_sim))

    # Blend (baseline stabilizes)
    b = float(blend_baseline)
    b = min(max(b, 0.0), 1.0)
    out = (1.0 - b) * knn_df.astype(float) + b * baseline_df.astype(float)
